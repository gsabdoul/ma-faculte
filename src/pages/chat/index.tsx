...
export default function Chat() {
    ...
    if ((message.trim() || attachedFiles.length > 0) && !isLoading && userId) {
      ...
      try {
        ...
        const { data, error } = await supabase.functions.invoke('chat-ai', {
          body: {
            messages: [...messages, { text: userText || '📎 Fichiers joints', sender: 'user' }],
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-5rem)] bg-white">
      {/* En-tête avec le bouton d'historique */}
      ...
      {isHistoryOpen && (
        <div className="fixed inset-y-0 left-0 w-80 bg-white shadow-lg z-20 p-4 transform transition-transform duration-300 ease-in-out flex flex-col">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-semibold">Historique des chats</h2>
            <button
